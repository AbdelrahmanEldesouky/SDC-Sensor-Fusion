...
class Sensor:
    ...
    def in_fov(self, x):
        ...
        # Convert position from vehicle to sensor coordinates
        pos_veh = np.ones((4, 1))  # homogeneous [px, py, pz, 1]
        pos_veh[0:3] = x[0:3]
        pos_sens = self.veh_to_sens * pos_veh  # transform to sensor frame

        # Assume that the object is not visible
        visible = False
    ...
    def get_hx(self, x):
        ...
        if self.name == "lidar":
            ...
            # Check before division if pos_sens[0] is zero
            if pos_sens[0] == 0:
                raise NameError("Division by zero!")

            # Project from sensor to image coordinates
            h_X = np.zeros((2, 1))
    ...
    def generate_measurement(self, num_frame, z, meas_list):
        ...
        # Add camera to supported sensors
