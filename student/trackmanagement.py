...
class Track:
    ...
    def __init__(self, meas, id):
        ...
        # Transform the measurement's position z from sensor to vehicle coordinates
        pos_sens = np.ones((4, 1))
        pos_sens[0:3] = meas.z[0:3]
        pos_veh = meas.sensor.sens_to_veh * pos_sens

        # Initialize the state vector (6x1)
        self.x = np.zeros((6, 1))
        self.x[0:3] = pos_veh[0:3]

        # Construct the positional part of P from measurement covariance
        P_pos = M_rot * meas.R * np.transpose(M_rot)

        # Construct the velocity part of P
        P_vel = np.matrix(
            [
        ...
        # Combine into the full 6x6 covariance P
        self.P = np.zeros((6, 6))
        self.P[0:3, 0:3] = P_pos
        self.P[3:6, 3:6] = P_vel

        # Initialize the track state and score
...
class Trackmanagement:
    ...
    def manage_tracks(self, unassigned_tracks, unassigned_meas, meas_list):
        ...
        for i in unassigned_tracks:
            ...
            if meas_list:  # if not empty
                if meas_list[0].sensor.in_fov(track.x):
                    ...
                    # Prevent score from going too high before subtracting
        ...
        for track in self.track_list:
            ...
            #  - track is confirmed but score < delete_threshold
            #  - track's covariance in x or y direction exceeds params.max_P
            if (
    ...
    def handle_updated_track(self, track):
        ...
        # Increase the track score by 1/window
        track.score += 1.0 / params.window
        
        # Check if it crosses the threshold to become 'confirmed'
